  ...
  <body>
      ...
      <header>
	   ...
	   ?>
  	   ...
  	   <?php
			...
			$temp_table = "sandbox.map_temp_$RAND";
			...
			$dbconn = pg_connect("dbname='cabi' host=$host port=5432 user='datauser' password='cabidata' connect_timeout=5") ;
			pg_query("set time zone 'America/New_York';");// set the timezone 
		
			$ping = pg_fetch_result(pg_query("select 'connected to db' as ping"),'0','ping');
			echo "<!-- $ping-->";
			
			// drop the temp table & recreate 
			pg_query("BEGIN");
				
			pg_query("drop table if exists $temp_table;") or die('<br><b>Drop failed: ' .  pg_result_error_field().'</b>');
			...
			$system_wide = pg_fetch_result(pg_query("select sum(n_rides) as total_rides from $temp_table"), 0, 'total_rides');
			
			
			
			$file_name = "rides_export_$RAND";


		// create file with the data 
			$cols = pg_query("select column_name from information_schema.columns where table_name = 'map_temp_$RAND' order by ordinal_position");
			$cols = pg_fetch_all($cols);
				
			$data_title = array();
			$col_array = array();
			foreach($cols as $col ){
				$col_array[] = $col['column_name'];
				$data_title[] = '{title: '.$col['column_name'].'}';
			}
			$col_str = implode("\t",$col_array)."\r\n";
			$data_title= implode(",",$data_title);
				
				
			$rows = pg_copy_to($dbconn, "$temp_table", "\t", ''); //(arguments are connection, table name, delimiter, null character )
			
			array_unshift($rows, $col_str); // put the column name string at the beginning of the array
			$file_path = "/var/www/html/temp/";
			//$file_name = $_REQUEST['file_name'] ?: "query_results_$RAND";
			//$file_name = str_replace('/','_',$file_name );
			file_put_contents($file_path.$file_name.'.txt', $rows ); // write to file 
	...
	?>
	  ...
	  <p>
	   ...
	   <a href="./temp/<?php echo $file_name?>.txt" >Download this data</a><br>
	   <div id="map"></div>
       </p>
		...
		var map = L.map('map',{ center:  [38.912522, -77.031382], zoom: 14, minZoom: 12, maxZoom: 19});  
	  ...
	  <p>
	  The full dataset of Cabi rides is available on an AWS database! Use <a href='http://www.pgadmin.org' target='_blank'>pgAdmin III</a> or the <a href='http://initd.org/psycopg/docs/' target='_blank'>psycopg2</a> python library to connect. Click <a href='credentials.txt'>here</a> to get the login credentials. 
	  </p>
	  <hr>
	   <footer>
