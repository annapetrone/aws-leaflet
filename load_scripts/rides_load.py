...
while t <= end_date:
	...
	if is_loaded==0:
		...
		print(has_columns)
		cur.copy_from(f,'rides',columns=has_columns,sep=',')
		conn.commit()
		f.close()
		...
		update_sql = open(sqldir+'rides_time_period_update.sql','r').read().split(';')
		[cur.execute(sql) for sql in update_sql if len(sql)>5]
		conn.commit()

		""" # skipping for now, currently not used
		if 'duration_str' in has_columns:
			print('parsing duration text')
			update_sql = open(sqldir+'rides_duration_update.sql','r').read().split(';')
			[cur.execute(sql) for sql in update_sql if len(sql)>5]
			conn.commit()
		"""

		""" # takes too long- only add to summary table
		if 'start_terminal_id' not in has_columns:
			print('updating terminal id fields')
			update_sql = open(sqldir+'rides_station_id_update.sql','r').read().split(';')
			[cur.execute(sql) for sql in update_sql if len(sql)>5]
			conn.commit()
		""" 
